...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python parse_pcap.py <pcap_file> [output_format]")
        print("  output_format: json, csv, or both (default: summary only)")
        print("\nExample:")
        print("  python parse_pcap.py ../data/PCAP_1_H1.pcap json")
        sys.exit(1)
    ...
    if output_format == 'json' or output_format == 'both':
        parser.export_to_json(f"{base_name}_parsed.json")
    
    if output_format == 'csv' or output_format == 'both':
        parser.export_to_csv(f"{base_name}_parsed.csv")
    
    # Print some sample queries and responses
    if parser.dns_queries:
        print("\nSample DNS Queries:")
        for i, query in enumerate(parser.dns_queries[:3], 1):
            print(f"\n  Query {i}:")
            print(f"    Transaction ID: {query['transaction_id']}")
            print(f"    Source: {query['src_ip']}:{query['src_port']}")
            print(f"    Destination: {query['dst_ip']}:{query['dst_port']}")
            if query['queries']:
                print(f"    Query: {query['queries'][0]['name']} ({query['queries'][0]['type_name']})")
    
    if parser.dns_responses:
        print("\nSample DNS Responses:")
        for i, response in enumerate(parser.dns_responses[:3], 1):
            print(f"\n  Response {i}:")
            print(f"    Transaction ID: {response['transaction_id']}")
            print(f"    Source: {response['src_ip']}:{response['src_port']}")
            print(f"    Destination: {response['dst_ip']}:{response['dst_port']}")
            if response['answers']:
                print(f"    Answer: {response['answers'][0]['name']} -> {response['answers'][0]['rdata']}")
